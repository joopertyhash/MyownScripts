...
def tg_msg_monitor():
    try:   
        ...
        while True:
            try:  
                url = f'https://api.telegram.org/bot{api_key}/forwardMessage?chat_id={my_chat_id}&from_chat_id={from_chat_id}&disable_notification=true&message_id={str(i)}'
                x = requests.get(url)
                if x.status_code == 200:
                    resp = json.loads(x.text)
                    text_message = resp["result"]["text"]
                    print("message_id =", i, x, text_message)
                    i=i+1
                    last_message = i
